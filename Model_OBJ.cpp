...
void Model_OBJ::Load(const char* filename, GLuint textureID) {
    ...
    while (std::getline(file, line)) {
        ...
        if (type == "v") {
            ...
            // Read whole line to detect Quads (4 vertices) vs Triangles (3)
            std::vector<std::string> segments;
            std::string segment;
            while(iss >> segment) segments.push_back(segment);
            
            // If Quad (4 pts), split into 2 triangles: (0,1,2) and (0,2,3)
