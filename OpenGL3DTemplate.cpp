...
int texCrystal;
int texTrap;
int texPlayer; // Player Texture

// 3DS Chunk IDs (Cleaned up spaces)
...
void Keyboard(unsigned char key, int x, int y) {
	...
	if (levelComplete || gameOver) return; // Freeze controls
	...
	case 'w':
	case 'W':
		playerX += d * sin(playerRot * 3.14159 / 180);
		playerZ += d * cos(playerRot * 3.14159 / 180);
		break;
	case 's':
	case 'S':
		playerX -= d * sin(playerRot * 3.14159 / 180);
...
void Display(void) {
	...
	for (size_t i = 0; i < traps.size(); i++) {
		glPushMatrix();
		glTranslatef(traps[i].x, 1.5f, traps[i].z);
...
void main(int argc, char** argv) {
	...
	glutCreateWindow("ROBO COLLECTOR - Level 1 FINAL");
	...
	texPyramid = LoadTexture("pyramid.bmp", 255);
	modelPyramid.tex = texPyramid;

	texPortal = LoadTexture("portal.bmp", 255);
	modelPortal.tex = texPortal;

	texCrystal = LoadTexture("crystal.bmp", 255);
	// modelCrystal.tex = texCrystal; // DISABLED

	texTrap = LoadTexture("trap.bmp", 255);
	modelTrap.tex = texTrap;

	// UPDATED: Load robot.bmp
	texPlayer = LoadTexture("robot.bmp", 255);
	modelPlayer.tex = texPlayer;

	// --- INITIALIZE GAME OBJECTS (RANDOMIZED) ---
	// Generate 50 crystals in a 600x600 area
	for (int i = 0; i < 50; i++) {
		GameObject c;
		c.x = (rand() % 600) - 300; // -300 to 300
		c.z = (rand() % 600) - 300;
		c.active = true;
	...
	// 5 Traps
	float tPos[5][2] = {
		{0, -100}, {0, -150}, {50, -120}, {-50, -120}, {0, 50}
	};
	for (int i = 0; i < 5; i++) {
		GameObject t;
		t.x = tPos[i][0];
